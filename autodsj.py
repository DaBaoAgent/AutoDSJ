...
def _visual_stats(folder: Path) -> dict:
    ...
    if plan_path.exists():
        try:
            ...
            plan_matches = False
    ready = (0 < frame_count <= 240 and success == frame_count and selective_schema
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p_run.add_argument("--target-frames", type=int, default=0,
                       help="选择性云端视觉复核帧数（60-240；默认0=按风险自动选择）")
    ...
    p_prepare.add_argument("--target-frames", type=int, default=0,
                           help="云端视觉帧数（60-240；默认0=按风险自动选择）")
    p_prepare.add_argument("--workers", type=int, default=3,
                           help="抽帧与视觉识别并发（默认3）")
    p_prepare.set_defaults(func=cmd_prepare)

    p_visual = sub.add_parser("visual", help="候选驱动云端视觉复核（风险自适应60–240帧）")
