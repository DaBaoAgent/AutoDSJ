...
def audit_episode(folder: Path) -> dict:
    """对单集执行完整质检."""
    match = load_json(folder / "_DY工作文件" / "★ 匹配报告.json")

    # ── 加载辅助数据（优先 _DY工作文件/，回退根目录） ──
    ...
    vi_path = _find_json("_DY工作文件/_source_visual_index.json", "_source_visual_index.json")
    frames = load_json(vi_path).get("frames", []) if vi_path else []

    ad_path = _find_json("_DY工作文件/_source_ad_intervals.json", "_source_ad_intervals.json")
    ad_intervals = []
    if ad_path:
        ad = load_json(ad_path)
        ad_intervals = [(i["start"], i["end"]) for i in ad.get("intervals", [])]

    sm_path = _find_json("_DY工作文件/_scene_map.json", "_scene_map.json")
