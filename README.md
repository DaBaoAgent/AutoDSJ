...
`文案审校 → 字幕/剧本索引 → 完整大场景地图 → 事件/物理镜头索引 → 父段全局序列解码 → 30～60帧选择性视觉复核 → 场景门禁 → Qwen 克隆配音 → 渲染 → 发布交付门禁`
...
├── ★ 发布信息.txt
├── ★ 剪映字幕导入.txt
└── _DY工作文件/                 # 场景图、索引、报告、TTS、SRT及交付清单
```

成片完成后，单集根目录只保留原片、原片字幕、源文案、成片、发布信息、剪映字幕导入文件和 `_DY工作文件` 这一个工作目录。再次运行处理命令时会自动把工作文件恢复到根目录供旧内核复用，交付完成后重新归档。不要把最终成片当作新原片。
