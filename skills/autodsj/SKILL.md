...
- 动作句的人工 `override` 必须锚定动作起点而非动作后的对白/反应字幕；渲染前抽取该源片范围和对应成片范围的连续帧，确认动作起势、发生、结果都落入计划片段。只看到动作结果（例如人已在水里）不能算命中“推入水里”。
- 独立 `quality` 复查必须按 `_AutoDSJ工作文件`、素材根目录、旧 `_DY工作文件` 的顺序寻找最新报告与索引；`HIGH` 人物缺失是人工复核告警，不能替代候选门禁，也不能仅凭云端未写角色名就判定真实错配。
