...
```powershell
cd "$HOME\AutoDSJ"
.\.venv\Scripts\python.exe autodsj.py set-key --dashscope "<新电脑自己的Key>"
...
另一台电脑仍必须遵守唯一管线：完整场景地图、字幕/剧本混合检索、CAM++ 可选声纹、父段全局序列解码、60～240 帧风险自适应云端视觉复核、广告硬禁区、画面去重、统一 -16 LUFS 等响、发布交付门禁和 `_AutoDSJ工作文件` 自动归档。不得因为换电脑而降级为旧匹配器或固定间隔密集全片视觉扫描。
